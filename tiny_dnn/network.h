...
network &operator<<(network &n, Layer &&l);
...
public:
   ...
   **/
    bool has_same_weights(const network &rhs, float_t eps) const {
        ...
        {
            if(!(*first1)->has_same_weights(**first2, eps))
            {
                return false;
            }
        }
...
protected:
    ...
    template <typename Layer>
    friend network &operator<<(network &n, Layer &&l);
...
std::basic_ostream<Char, CharTraits> &operator<<(
        std::basic_ostream<Char, CharTraits> &os, const network &n)
{
    n.save(os);
    return os;
}
...
std::basic_istream<Char, CharTraits> &operator>>(
        std::basic_istream<Char, CharTraits> &os, network &n)
{
    n.load(os);
    return os;
}
